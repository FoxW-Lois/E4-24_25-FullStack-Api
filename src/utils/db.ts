...
export async function seedDatabase() {
    ...
    if (roles.length == 0) {
        let leader = new DbRole({
            name: "leader",
            permissions: [{ resources: ["participant", "po", "sm"], actions: ["add", "remove"] }]
        });
        await leader.save();

        const po = new DbRole({
            name: "po",
            permissions: [{ resources: ["story"], actions: ["create", "delete", "priorize"] }]
        });
        await po.save();
        ...
        const sm = new DbRole({
            name: "sm",
            permissions: [
                { resources: ["sprint"], actions: ["create", "update", "delete", "read", "list"] },
                { resources: ["story"], actions: ["move", "remove"] }
            ]
        });
        await sm.save();
        ...
        const participant = new DbRole({
            name: "participant",
            permissions: [
                { resources: ["task"], actions: ["create", "update", "delete"] },
                { resources: ["story"], actions: ["estimate"] }
            ]
        });
